    ...
    async initialize(connection: Connection) {
        ...
        await connection.manager.findAndCount(User)
            .then(async (count) => {
                ...
                if (count[1] == 0) {
                    connection.manager.save(connection.manager.create(User, user))
                    ...
                    // question questionAAA
                    const questionABA: Question = new Open("questionABA", "problemstatement", "correctanswer");
                    await connection.manager.save(questionABA)
                        .then(() => this.logger.log(`Created example ${questionABA.title}`))
                        .catch((error) => this.logger.error(error));

                    // question questionAAB
                    const questionABB: Question = new MultipleChoice("questionABB", "problemstatement", "correctanswer", ["a", "b"]);
                    await connection.manager.save(questionABB)
